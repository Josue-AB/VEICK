function InfoModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#f0f5fc] rounded-2xl max-w-3xl w-full max-h-[92vh] overflow-y-auto relative shadow-2xl">

        {/* Cerrar */}
        <button onClick={onClose}
          className="absolute top-4 right-4 w-9 h-9 rounded-full border border-blue-200 bg-white text-sm flex items-center justify-center z-10 hover:bg-[#1a3f7a] hover:text-white transition-all hover:rotate-90">
          ✕
        </button>

        {/* HERO */}
        <div className="bg-[#1a3f7a] rounded-t-2xl px-9 py-10 relative overflow-hidden">
          ...
          <h2 className="text-3xl font-black text-white leading-tight mb-2">
            ...
            <span className="text-blue-300">Lengua de Señas Mexicana</span>
          </h2>
        ...
        <div className="px-9 py-8 space-y-8">
          ...
          {/* PASOS */}
          <div>
            <p className="text-[10px] font-bold tracking-[0.14em] uppercase text-[#1a3f7a] mb-4 flex items-center gap-2 after:content-[''] after:flex-1 after:h-px after:bg-blue-200">¿Cómo funciona?</p>
            <div className="grid grid-cols-3 gap-3">
              {steps.map(s => (
                <div key={s.num} className="bg-white border border-blue-100 rounded-xl p-4 hover:shadow-md hover:-translate-y-0.5 transition-all">
                  <span className="text-[11px] font-bold text-[#1a3f7a] tracking-wide">PASO {s.num}</span>
          ...
          {/* IMÁGENES */}
          <div>
            <p className="text-[10px] font-bold tracking-[0.14em] uppercase text-[#1a3f7a] mb-4 flex items-center gap-2 after:content-[''] after:flex-1 after:h-px after:bg-blue-200">Dataset de señas</p>
            <div className="grid grid-cols-3 gap-3">
              {images.map((img, i) => (
                <div key={i} className="bg-white border border-blue-100 rounded-xl overflow-hidden hover:scale-[1.02] transition-transform">
                  <img src={img.src} alt={img.label}
                    className="w-full aspect-[4/3] object-cover"
                    onError={e => { e.target.style.display='none'; e.target.nextSibling.style.display='flex'; }}
                  />
                  <div className="hidden aspect-[4/3] bg-[#e6f1fb] items-center justify-center flex-col gap-1 text-3xl">
                    {img.emoji}
                    <span className="text-[10px] text-[#1a3f7a] opacity-60">{img.src}</span>
                  </div>
                  <div className="p-3">
          ...
          {/* TECNOLOGÍAS */}
          <div>
            <p className="text-[10px] font-bold tracking-[0.14em] uppercase text-[#1a3f7a] mb-3 flex items-center gap-2 after:content-[''] after:flex-1 after:h-px after:bg-blue-200">Tecnologías empleadas</p>
            <div className="flex flex-wrap gap-2">
              {techs.map(t => (
                <span key={t} className="bg-white border border-blue-100 rounded-full px-3 py-1.5 text-[12px] text-stone-800 font-medium hover:border-[#1a3f7a] hover:bg-[#e6f1fb] transition-colors">{t}</span>
              ))}
            </div>
          </div>

          {/* IMPACTO */}
          <div className="bg-[#e6f1fb] border border-blue-200 rounded-xl p-4 flex gap-3">
            <span className="text-xl">♿</span>
            <p className="text-[12.5px] text-[#1a3f7a] leading-relaxed">
